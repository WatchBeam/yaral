...
exports.register = function (server, options, next) {
    ...
    const resolveRouteOpts = (req) => {
        const routeOpts = req.route.settings.plugins.yarl;

        let opts = { enabled: true, buckets: [] };
        if (!routeOpts) {
            // do nothing
        } else if (typeof routeOpts === 'string') { // specifying bucket as string
            opts = { enabled: true, buckets: [routeOpts] };
        ...
        if (opts.enabled) {
            opts.enabled = opts.buckets.length + options.default.length > 0;
    ...
    server.ext('onPreResponse', (req, reply) => {
        ...
        const opts = req.plugins.yarl && matchBucket(req.plugins.yarl, res);
        if (!opts || req.plugins.yarl.limited) {
            return reply.continue();
        }
